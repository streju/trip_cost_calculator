...
GroupModel::GroupModel(WarningIndicatorPtr warningIndicator, const CurrentStateProviderPtr& currentStateProvider,
    ...
    : warningIndicator_(warningIndicator), currentStateProvider_(currentStateProvider), queryCreator_(queryCreator)
{
...
{
    queryCreator_->create("CREATE TABLE IF NOT EXISTS groups ("
    ...
    warningIndicator_->setWarning("", WarningType::GROUP_CREATION);
    refreshModel();
    return true;
}
...
void GroupModel::assignUserToGroup(const QString groupName, const QString user)
{
    ...
    while (query->next())
    {
        const auto addingQuery = queryCreator_->create();
