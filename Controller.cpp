...
Controller::Controller(const UserValidationPtr& userValidation, GroupDetails* const groupDetails, FriendsModelPtr friendsModel,
    ...
    WarningIndicatorPtr warningIndicator, CurrentStateProviderPtr currentStateProvider, QObject *parent)
    : QObject(parent),
    ...
    registrationService_(std::make_shared<RegistrationService>(warningIndicator))
{
}
...
void Controller::removeFriend(const QString& userToRemove)
{
    friendsModel_->removeFriend(userToRemove);
}
